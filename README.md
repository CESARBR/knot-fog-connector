...
* `cb` **Function** event handler defined as `cb(id, sensorId)` where:
  * `id` **Number** device ID (KNoT ID)
  * `sensors` **Array** IDs of the sensor to send updated data (**Number**)
...
* `cb` **Function** event handler defined as `cb(id, sensorId, data)` where:
  * `id` **Number** device ID (KNoT ID)
  * `updates` **Array** update for each actuator, each one formed by:
    * `actuatorId` **Number** ID of the actuator to update
